...
### 6.5 Settings
- Manage categories: **create (inline from Quick Add, §6.2), rename, emoji, reorder**. The `slug` (§5.3) is immutable and is not exposed in the UI. Category **deletion is not offered in v1** (§5.3). *(The "rename, emoji, reorder only" wording is superseded by Amendment 8 — 18 Aug 2026; see `CHANGELOG.md`.)*
