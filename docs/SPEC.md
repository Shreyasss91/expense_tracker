...
5. **Session revocation on password change (owner request, 13 Sep 2026):** the session is a **stateless JWT signed with `AUTH_SECRET`**, so a cookie issued under an old password would otherwise keep verifying until `maxAge` — and `updateAge` would roll an active holder forward indefinitely. **Normative:** the `jwt` callback pins an **HMAC-SHA-256 fingerprint of `FAMILY_MASTER_PASSWORD` — keyed by `AUTH_SECRET`**, so the value in the token is not brute-forceable back to the password — into the token at sign-in and **re-derives it on every request**; on mismatch the `session` callback **drops `user`**, which makes middleware redirect pages to `/login` and makes every `if (!session?.user)` Server Action / route-handler guard fail closed. This is the sanctioned password-change mechanism, unchanged from §6.5: **update the env var, then redeploy — that signs out every existing session.** Middleware is bundled for the Edge runtime, where Next inlines `process.env` at build time, so an env change **without** a redeploy does not take effect. Because the fingerprint lives in the token, **`AUTH_SECRET` rotation alone also retires every session**.
