...
6. **Category:** grid of categories (emoji + name) with per-category remaining-budget hints (§6.7) — tapping **selects** the category (highlighted, with a check); it no longer commits. Recently used categories float to the top of the grid (per-device `localStorage`, recorded on each successful commit); never-used categories keep the manual order from Settings (§6.5).
