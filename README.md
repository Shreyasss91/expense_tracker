...
- **Quick Add** (§6.2): a single bottom sheet — amount text input, tag chips, date/time, note, and a category grid (tap to select) with an **Add transaction** button that commits. Supports inline category rename/emoji edit. Amounts are integer paise (§5.8); times normalize `HH:MM` → `HH:MM:00` (§5.6); the active member comes from the `active_member_id` cookie and is validated against the `members` table (§3.2).
